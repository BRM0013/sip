         ...
         {{-- DATA GRID --}}
            <div class="col-md-12 form-inline main-layer panelSearch p-10">
              <div class="form-group pull-left">
                <a class="btn btn-warning" href="{{ route('add_users') }}">
                  <i class="fa fa-plus-square iconLabel" style="margin-right: 15px"></i> Tambah
                </a>
                <a class="btn btn-success" href="javascript:void(0)" onclick="Kirim_verifikasi_ulang()">
                  <i class="fa fa-whatsapp iconLabel" style="margin-right: 15px"></i> Kirim Verifikasi Ulang
                </a>
              </div>
...
@section('js')
  <script type="text/javascript">
    ...
    function get_load(url) {
      $("#datagrid").html('');
        ...
        columns                 : [
          ...
          {field: 'status_verifikasi', title: 'Status', editable: false, sortable: true, width: 200, align: 'left', search: true},
    ...
    function menu(rowData,rowIndex){
      ...
      html +=     '<li><a href="{{ url("/") }}/home/users/edit/'+rowData.id+'"><i class="fa fa-pencil"></i> Edit</a></li>';
      html +=     '<li><a href="javascript:void(0)" onclick="daftar_sip_diluar(\''+rowData.id+'\')"><i class="fa fa-list-alt"></i> Daftar SIP</a></li>';
    ...
    function deletes(id) {
      swal({
       ...
       function(){
         $.post("{{route('delete_users')}}", {id:id}).done(function(data){
    ...
    function Kirim_verifikasi_ulang() {
      $.post("{{ route('kirim_verifikasi_ulang') }}").done((data) => {
        if (data.status == 'success') {
          swal('Success',data.message,'success');
        }
      })
