  ...
  <script type="text/javascript">
    ...
    function get_load(url) {
      ...
      var datagrid = $("#datagrid").datagrid({
        ...
        columns                 : [
          {field: 'id_provinsi', title: 'Kode Provinsi', editable: false, sortable: true, width: 200, align: 'left', search: true},
    ...
    $(document).ready(function() {
      get_load('{{ route('datagrid_master_provinsi') }}');
    });
    ...
    function menu(rowData,rowIndex){
      ...
      html +=     '<li><a href="javascript:void(0);" onclick="formInput(\''+rowData.id_provinsi+'\')"><i class="fa fa-pencil"></i> Edit</a></li>';
      // html +=     '<li><a href="javascript:void(0);" onclick="deletes(\''+rowData.id_provinsi+'\')"><i class="fa fa-trash-o"></i> Delete</a></li>' +
                '</ul>' +
              '</div>';
      return html;
    }
    ...
    function formInput(id){
      $.post("{!! route('add_provinsi') !!}", {id:id}).done(function(data){
    ...
    function deletes(id) {
      swal({
       ...
       function(){
         $.post("{{route('delete_provinsi')}}", {id:id}).done(function(data){
           if(data.status == 'success'){
             datagrid.reload();
