...
<?php
  ...
  if (isset($users)) {
    ...
    if (!empty($users->fasyankes_id)) {
      $fasyankes_id           = $users->fasyankes_id;          
    }
  }
?>
    ...
    <div class="box" style="border-top:none">
      ...
      <div class="col-md-10">
        ...
        <h4 style="padding: 10px;margin:0px;color:#fff;font-weight: 600;background: #b6d1f2; text-shadow: 0 1px 0 #aecef4;">
          <i class="fa fa fa-cog iconLabel m-r-15"></i> Data Pengguna Rumah Sakit
        </h4>
          <div class="box-body">
            <div class="col-md-12">
              <form action="{{ route('save_data_rs') }}" method="post" enctype="multipart/form-data">
               @csrf
              ...
              <div class="row">                
                <div class="col-md-12">
                  <div class="form-group">
                    <label class="col-sm-12 control-label">Nama Rumah Sakit <small style="color: red;">*</small></label>                                      
                    <div class="col-sm-12">
                      <input type="text" name="name" value="{{ $name }}" class="form-control disablecopypaste" placeholder="Nama Rumah Sakit" style="text-transform:uppercase" onkeypress="return cekHuruf(event)">
                    </div>                    
                  </div>
                  ...
                  <?php if ($setting != 'setting'): ?>                                        
                    <div class="col-sm-12">
                      <label>Jenis Fasyankes / Rumah Sakit<small style="color: red;">*</small></label>
                      <select name="fasyankes_id" class="form-control select2"  style="text-transform:uppercase">
                        <option value="">..:: Pilih Jenis Organisasi Profesi ::..</option>
                        <?php foreach ($list_fasyankes as $row): ?>
                          <?php if (!empty($fasyankes_id)): ?>
                            <?php if ($row->id_fasyankes == $fasyankes_id): ?>
                              <option value="{{ $row->id_fasyankes }}" selected="selected">{{ $row->nama }}</option>                          
                            <?php endif ?>
                          <?php endif ?>
               ...
               <div class="box-footer">
                ...
                @if($setting=='')
                <a href="{{route('data_rs')}}" class="btn btn-warning btn-cencel pull-right">
