  ...
  <section class="content">
      <div class="box box-info" id="btn-add" style="border-top:none">
        <h4 style="padding: 10px;margin:0px;color:#fff;font-weight: 600;background: #b6d1f2; text-shadow: 0 1px 0 #aecef4;">
          <i class="fa fa-file iconLabel m-r-15"></i> Data Jabatan
        </h4>
      <div class="box-body">
        <div class="col-md-12">
         {{-- DATA GRID --}}
...
@section('js')
  <script type="text/javascript">
    ...
    $(document).ready(function() {
      get_load('{{ route('datagrid_jabatan') }}');
    });
    ...
    function menu(rowData,rowIndex){
      ...
      html +=     '<li><a href="javascript:void(0);" onclick="formInput(\''+rowData.id_jabatan+'\')"><i class="fa fa-pencil"></i> Edit</a></li>';
      html +=     '<li><a href="javascript:void(0);" onclick="deletes(\''+rowData.id_jabatan+'\')"><i class="fa fa-trash-o"></i> Delete</a></li>' +
                '</ul>' +
              '</div>';
      return html;
    }
    ...
    function formInput(id){
      $.post("{!! route('add_jabatan') !!}", {id:id}).done(function(data){
    ...
    function deletes(id) {
      swal({
       ...
       function(){
         $.post("{{route('delete_jabatan')}}", {id:id}).done(function(data){
           if(data.status == 'success'){
             datagrid.reload();
